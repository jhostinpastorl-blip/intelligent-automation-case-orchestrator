...
class ExecutionWorker:
    def run_once(self):
        ...
        if record.analysis is None or record.analysis.proposed_tool is None:
            ...
            store.save(record)
            store.event(record, "execution_failed", "No tool")
            return True
        ...
        with get_tracer().start_as_current_span("execute_approved_intent") as span:
            ...
            except Exception as exc:  # noqa: BLE001 - worker must persist arbitrary execution failures
                record.execution_result = {"error": str(exc)}
                record.status = CaseStatus.FAILED
                store.save(record)
                store.event(record, "execution_failed", str(exc))
                span.record_exception(exc)
            else:
                record.execution_result = result
                record.status = CaseStatus.EXECUTED
                store.save(record)
                store.event(record, "tool_executed", record.analysis.proposed_tool.tool_name)
        return True
...
def main():
    ...
    worker.run_forever(float(os.getenv("WORKER_POLL_INTERVAL_SECONDS", "1")))
