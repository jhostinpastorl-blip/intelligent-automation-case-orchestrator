...
class DispatchQueue:
    @property
    def brokered(self): return False
    def publish(self, case_id: str): return None
    def consume(self, timeout_seconds: int = 1): return None
...
class RedisDispatchQueue(DispatchQueue):
    ...
    @property
    def brokered(self): return True
    def publish(self, case_id: str): self.client.rpush(self.queue, case_id)
...
def get_dispatch_queue():
    return RedisDispatchQueue() if os.getenv("DISPATCH_BACKEND", "database").lower()=="redis" else DispatchQueue()
